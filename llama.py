...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        return x / (mean_squared + eps).sqrt()
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        scaled = attention_scores / math.sqrt(self.head_dim)
        attention_weights = F.softmax(scaled, dim=-1)
        
        # dropout
        attention_weights = self.attn_dropout(attention_weights)
        output_scores = torch.matmul(attention_weights, value)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_output = self.attention(norm)
        res_output = x + attn_output
        norm_attn_output = self.ffn_norm(res_output)
        output = self.feed_forward(norm_attn_output)
        output += res_output
        return output
